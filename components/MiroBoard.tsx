...
export default function MiroBoard() {
  ...
  }, [draggedNote, draggedText, draggedImage, dragOffset, isDrawing, tool, getMousePosition, isDragging, dragStart, scrollX, scrollY, scale, lastTouchDistance, BOARD_WIDTH, BOARD_HEIGHT]);
  ...
  }, [draggedNote, draggedText, draggedImage, isDrawing, currentPath, selectedColor, brushSize]);
    ...
    if (resizingNote) {
      setStickyNotes(prev =>
        prev.map(note => {
          ...
          return {
            ...note,
            x: Math.max(0, Math.min(newX, BOARD_WIDTH - newWidth)),
            y: Math.max(0, Math.min(newY, BOARD_HEIGHT - newHeight)),
            width: Math.min(newWidth, BOARD_WIDTH - newX),
            height: Math.min(newHeight, BOARD_HEIGHT - newY)
          };
        })
      );
  ...
  return (
    ...
    >
      ...
      <div className="bg-gradient-to-r from-gray-50 to-gray-100 border-b border-gray-200/50 transition-all duration-300">
        <div className="p-3 sm:p-4">
          <div className="flex flex-col space-y-3">
            {/* Main tools - optimized for mobile */}
      ...
      >
        ...
        >
          {/* Send Email Button - Fixed position on board */}
          <button
            onClick={sendEmail}
            disabled={isEmailSending}
            className={`fixed top-4 right-4 z-50 px-4 py-2 sm:px-6 sm:py-3 rounded-full transition-all duration-200 font-medium whitespace-nowrap flex items-center space-x-2 shadow-lg backdrop-blur-sm ${
              isEmailSending
                ? 'bg-gray-100/90 text-gray-400 cursor-not-allowed'
                : 'bg-blue-500/90 text-white hover:bg-blue-600/90 hover:scale-105'
            }`}
            style={{
          ...
          {imageElements.map(image => (
            <div
              key={image.id}
              className={`absolute group transition-all duration-200 ${tool === 'select' ? 'cursor-move hover:scale-105' : tool === 'eraser' ? 'cursor-not-allowed hover:opacity-50' : 'cursor-default'} ${draggedImage === image.id ? 'scale-105 shadow-xl z-10' : ''}`}
              style={{
                left: `${image.x}px`,
                top: `${image.y}px`,
            ...
            >
              ...
              {tool === 'select' && (
                <>
                  ...
                  {/* Resize handles */}
                  <div
                    className={`absolute -top-1 -left-1 ${isMobile ? 'w-4 h-4' : 'w-3 h-3'} bg-blue-500 rounded-full opacity-0 group-hover:opacity-100 transition-opacity cursor-nw-resize`}
                    onMouseDown={(e) => handleResizeStart(e, image.id, 'image', 'nw')}
                    onTouchStart={(e) => handleResizeStart(e, image.id, 'image', 'nw')}
                  ></div>
                  <div
                    className={`absolute -top-1 -right-1 ${isMobile ? 'w-4 h-4' : 'w-3 h-3'} bg-blue-500 rounded-full opacity-0 group-hover:opacity-100 transition-opacity cursor-ne-resize`}
                    onMouseDown={(e) => handleResizeStart(e, image.id, 'image', 'ne')}
                    onTouchStart={(e) => handleResizeStart(e, image.id, 'image', 'ne')}
                  ></div>
                  <div
                    className={`absolute -bottom-1 -left-1 ${isMobile ? 'w-4 h-4' : 'w-3 h-3'} bg-blue-500 rounded-full opacity-0 group-hover:opacity-100 transition-opacity cursor-sw-resize`}
                    onMouseDown={(e) => handleResizeStart(e, image.id, 'image', 'sw')}
                    onTouchStart={(e) => handleResizeStart(e, image.id, 'image', 'sw')}
                  ></div>
                  <div
                    className={`absolute -bottom-1 -right-1 ${isMobile ? 'w-4 h-4' : 'w-3 h-3'} bg-blue-500 rounded-full opacity-0 group-hover:opacity-100 transition-opacity cursor-se-resize`}
          ...
          {stickyNotes.map(note => (
            <div
              key={note.id}
              className={`absolute rounded-lg shadow-lg border border-gray-300 group transition-all duration-200 ${tool === 'select' ? 'cursor-move hover:scale-105' : tool === 'eraser' ? 'cursor-not-allowed hover:bg-red-200' : 'cursor-default'} ${draggedNote === note.id ? 'scale-105 shadow-xl z-10' : ''}`}
              style={{
                left: `${note.x}px`,
                top: `${note.y}px`,
                width: `${note.width}px`,
          ...
          {textElements.map(text => (
            <div
              key={text.id}
              className={`absolute group transition-all duration-200 ${tool === 'select' ? 'cursor-move hover:scale-105' : tool === 'eraser' ? 'cursor-not-allowed hover:opacity-50' : 'cursor-default'} ${draggedText === text.id ? 'scale-105 shadow-xl z-10' : ''}`}
              style={{
                left: `${text.x}px`,
                top: `${text.y}px`,
