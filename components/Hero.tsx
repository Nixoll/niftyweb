          ...
          <div className={`text-left transition-all duration-1000 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
            <div className="mb-8">
              ...
              <h1 className="text-5xl lg:text-7xl font-bold text-gray-900 mb-8 leading-tight">
                {heroTexts[currentText].title.split(' ').map((word, index) => (
                  <span key={index} className="inline-block">
                    {word === heroTexts[currentText].accent ? (
                    ...
                    {index < heroTexts[currentText].title.split(' ').length - 1 && ' '}
                  </span>
                ))}
              </h1>
